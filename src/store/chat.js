...
export const chatJS = defineStore("chat", {
    ...
    actions: {
        start() {
            if (!this.server) {
                this.server = new WebSocket((import.meta.env.VITE_WEBSOCKET_PROTOCOL || '__WEBSOCKET_PROTOCOL__') + "://" + (import.meta.env.VITE_SERVER_DOMAIN_NAME || '__SERVER_DOMAIN_NAME__') + "chat")
